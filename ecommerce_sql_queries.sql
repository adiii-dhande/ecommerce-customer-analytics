...
-- Expected: orders=53,132 | customers=5,000 | revenue=$138M | avg_order=$2,596
SELECT
...
-- Expected: Premium=$58M | Standard=$49M | Basic=$31M
SELECT
    c.segment,
...
SELECT
    gender,
    COUNT(*) AS total_customers,
    ROUND(COUNT(*)*100.0/SUM(COUNT(*)) OVER(), 1) AS gender_share_pct
FROM customers
GROUP BY gender
ORDER BY total_customers DESC;

-- ── 9. Cohort Retention Analysis (Window Functions) ──────────────────────────
WITH cohort_base AS (
    SELECT
        customer_id,
...
-- Expected: ~24% YoY revenue growth
WITH yearly AS (
    SELECT
...
SELECT
    order_year,
    revenue_m,
    LAG(revenue_m) OVER (ORDER BY order_year) AS prev_year_revenue_m,
    ROUND(
...
SELECT
    c.segment,
    ROUND(AVG(ct.total_spend), 0) AS avg_clv,
    ROUND(MAX(ct.total_spend), 0) AS max_clv,
    ROUND(MIN(ct.total_spend), 0) AS min_clv
FROM (
